...
data LandlockRulesetAttr = LandlockRulesetAttr { landlockRulesetAttrHandledAccessFs :: U64 }
  deriving (Show, Eq)
...
instance Storable LandlockRulesetAttr where
  ...
  peek ptr = LandlockRulesetAttr <$> landlock_ruleset_attr_peek_handled_access_fs ptr
  poke ptr attr = do
      landlock_ruleset_attr_poke_handled_access_fs ptr (landlockRulesetAttrHandledAccessFs attr)
...
foreign import capi unsafe "hs-landlock.h hs_landlock_restrict_self"
  _landlock_restrict_self :: CInt
                          -> U32
                          -> IO CLong

{- HLINT ignore landlock_restrict_self "Use camelCase" -}
landlock_restrict_self :: CInt
                       -> U32
                       -> IO ()
